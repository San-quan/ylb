        ...
        success: function(res){
            ...
            if(res.code == 200){
                ...
                for (var i=0; i<res.kfList.length; i++) {
                    ...
                    if(res.kfList[i].kf_hmbz || res.kfList[i].kf_hmbz !== null){
                        
                        // 有
                        var kf_hmbz_ = res.kfList[i].kf_hmbz;
                    }else{
                    ...
                    var $tbody_HTML = $(
                        '<tr>' +
                        '   <td>'+kf_title+'</td>' +
                        '   <td>'+res.kfList[i].kf_title_broswer+'</td>' +
                        '   <td>'+kf_hmbz_+'</td>' +
                        '   <td>'+kf_online+'</td>' +
                        '   <td>'+kf_model+'</td>' +
                        '   <td>'+kf_safety+'</td>' +
                        ...
                        '       <div>' +
                        '       <div style="display:block;margin-top:15px;cursor:pointer;">' +
                        '           <span class="light-tag" id="'+kf_id+'" title="上传客服二维码" data-toggle="modal" data-target="#kfQrcodeListModal" onclick="getKfQrcodeList(this);">上传</span>' + 
                        '           <a href="iplist.html?kf_id='+kf_id+'"><span class="light-tag">访客</span></a>' + 
...
function getKfQrcodeList(e) {
            ...
            if(res.code == 200){
                ...
                for (var i=0; i<res.kfQrcodeList.length; i++) {
                    ...
                    var $zm_tbody_HTML = $(
                        ...
                        '       <span class="light-tag" onclick="resetKfQrcode(this)" id="'+zm_id+'">重置</span>' +
                        '   </td>' +
                        '</tr>'
                    );
    ...
    // 上传客服二维码按钮隐藏的input需要设置kf_id用于传递给uploadKfQrcode.php
    $(".addKfQrcodeForm input[name='kf_id']").val(kf_id);
}
...
// 上传客服二维码并手动添加
function addKfQrcodeByUpload() {
    
    // 获取表单数据
    const djsStatus = $('select[name="djsStatus"]').val();
    const djs = $('input[name="djs"]').val();
    const clickimgStatus = $('select[name="clickimgStatus"]').val();
    const clickimgJumpUrl = $('input[name="clickimgJumpUrl"]').val();
    const kfqrcode_imgurl = $('input[name="kfqrcode_imgurl"]').val();
    const kf_id = $('#kfQrcodeListModal input[name="kf_id"]').val();

    if (!kfqrcode_imgurl) {
        showErrorResult('请上传二维码图片！');
        return;
    }
    ...
    $.ajax({
        type: "POST",
        url: "./addKfQrcodeByUpload.php?djsStatus="+djsStatus+"&djs="+djs+"&clickimgStatus="+clickimgStatus+"&clickimgJumpUrl="+clickimgJumpUrl+"&kfqrcode_imgurl="+kfqrcode_imgurl+"&kf_id="+kf_id,
        success: function(res){
            ...
            if(res.code == 200){
                ...
                showSuccessResult(res.msg);
                getKfQrcodeList({ id: kf_id });
                
                // 清空表单
                $('#kfQrcodeListModal input[name="djs"]').val('');
                $('#kfQrcodeListModal input[name="clickimgJumpUrl"]').val('');
                $('#kfQrcodeListModal input[name="kfqrcode_imgurl"]').val('');
                $('.upload-btn-text').text('+ 上传并解析二维码');
            }else{
    ...
    $.ajax({
        ...
        success: function(res){
            ...
            if(res.code == 200){
                ...
                $('#EditKfModal input[name="kf_title_broswer"]').val(res.kfInfo.kf_title_broswer);
                
                // 备注信息
                $('#EditKfModal input[name="kf_hmbz"]').val(res.kfInfo.kf_hmbz);
    ...
    $.ajax({
        ...
        success: function(res){
            ...
            if(res.code == 200){
                ...
                $('#EditKfQrcodeModal select[name="clickimgStatus"]').val(res.kfzmInfo.clickimgStatus);
                $('#EditKfQrcodeModal select[name="djsStatus"]').val(res.kfzmInfo.djsStatus);
                $('#EditKfQrcodeModal input[name="djs"]').val(res.kfzmInfo.djs);
                $('#EditKfQrcodeModal input[name="clickimgJumpUrl"]').val(res.kfzmInfo.clickimgJumpUrl);
                ...
                var $previewQrcode_HTML = $(
                    ...
                    '<div>' +
                    '   <div class="Re-upload reUpload" onclick="newUpload();" style="width:120px;"> + 更换二维码 </div>' +
                    '</div>'
                );
...
function initialize_getDomainNameList(module){
    ...
    $('#createKfModal input[name="kf_title_broswer"]').val('');
