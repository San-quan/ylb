    ...
    <body>
        ...
        <?php
            ...
            if($page_key) {
                ...
                // 查询当前 page_key 是否存在
                $checkKey = $db->set_table('ylbPlugin_h5chatPages')->find(['page_key' => $page_key]);
                ...
                if($checkKey) {
                    ...
                    // 检查当前页面是否已达到过期时间
                    $page_expire_time = $checkKey['page_expire_time'];
                    ...
                    // 访问限制检测（仅限手机打开）
                    if($checkKey['limitation'] == 2 && !preg_match('/Mobile|Android|iPhone|iPad|iPod|Windows Phone|webOS|BlackBerry/i', $_SERVER['HTTP_USER_AGENT'])) {
                    ...
                    // 访问限制检测（仅限微信内打开）
                    if($checkKey['limitation'] == 3 && strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') === FALSE) {
                    ...
                    // 访问限制检测（仅限QQ内打开）
                    if($checkKey['limitation'] == 4 && strpos($_SERVER['HTTP_USER_AGENT'], 'QQ/') === FALSE && strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') === FALSE) {
                    ...
                    // 访问限制检测（仅限抖音内打开）
                    if($checkKey['limitation'] == 5 && (strpos($_SERVER['HTTP_USER_AGENT'], 'aweme') === FALSE || strpos($_SERVER['HTTP_USER_AGENT'], 'Bytedance') === FALSE)) {
                    ...
                    // 入口域名
                    $page_rkym = $checkKey['page_rkym'];
                    
                    // 跳转到入口页面
                    jumpTo($folderNum,$page_rkym,'h5chatlandpage','pkey',$page_key);
                }else {
