    ...
    <?php
        ...
        if($page_key){
            ...
            if($getPageInfo){
                ...
                // 落地域名
                $page_ldym = $getPageInfo['page_ldym'];
                
                // 状态
                $page_status = $getPageInfo['page_status'];
                
                // 当前访问次数
                $page_pv = $getPageInfo['page_pv'];
                ...
                if($page_status == 1) {
                    ...
                    // 更新访问次数
                    $newPV = $page_pv + 1;
                    $db->set_table('ylbPlugin_h5chatPages')->update(
                        ['page_key' => $page_key],
                        ['page_pv' => $newPV]
                    );
                    
                    // 跳转到落地域名页面
                    $jumpToldym = dirname(dirname($page_ldym . $_SERVER['REQUEST_URI'])).'/?key='.$page_key;
                    header('Location:' . $jumpToldym . '&exportKey='.MD5(time()).'&signature='.hash('sha256', $page_key.time()));
                }else {
